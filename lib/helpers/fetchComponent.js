    ...
    githubGet('frontcraft/fronthack-components', 'src/style', (err, data, content) => {
      ...
      fs.ensureDir(baseStylesPath, (err) => {
        ...
        content.forEach((file, index) => {
          githubGet('frontcraft/fronthack-components', `src/style/${file}`, (err, data, fileContent) => {
            if (err) throw err
            fs.writeFile(`${baseStylesPath}/${file}`, fileContent, (err) => {
              if (err) throw err
              if (index === content.length - 1) cb(null)
            })
          })
        })
      })
