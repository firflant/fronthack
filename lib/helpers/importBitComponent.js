...
module.exports = (projectRoot, type, machinename, description, cb = () => null) => {
  ...
  shell.exec(`bit import frontcraft.fronthack/${type}s/${kebabCase} --path ${projectRoot}/src/${(type !== 'global') ? `${type}s/`: ''}${kebabCase} --skip-npm-install --ignore-package-json --ignore-dist`, {async: true, silent: true}, (code) => {
    if (code !== 0) {
