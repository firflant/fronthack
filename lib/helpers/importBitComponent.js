...
module.exports = (type, machinename, description, cb = () => null) => {
  ...
  shell.exec(`bit import fronthack.fronthack/${type}s/${kebabCase} --path src/${type}s/${machinename} --skip-npm-install --ignore-package-json --ignore-dist`, {async: true, silent: true}, (code) => {
    if (code != 0) {
      if (type === 'component') {
