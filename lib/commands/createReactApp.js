  ...
  prompt.get(initSchema, (err, result) => {
    ...
    shell.exec(`npx create-react-app ${result.name}`, { async: true }, (code) => {
      ...
      shell.exec('echo y | yarn eject', { async: true }, (code) => {
        ...
        shell.exec('yarn add sass-loader node-sass copy-webpack-plugin bem-modifiers', { async: true, silent: true }, (code) => {
          if (code != 0) throw new Error('Error: Adding node dependencies')
          shell.exec('yarn add --dev fronthack-scripts', { async: true, silent: true }, (code) => {
                      ...
                      fs.writeFile(`${currentPath}/src/index.js`, newContent, (err) => {
                        ...
                        fs.unlink(`${currentPath}/src/index.css`, (err) => {
                          ...
                          fs.unlink(`${currentPath}/src/App.css`, (err) => {
                            ...
                            fs.readFile(`${fronthackPath}/lib/templates/webpack.config.sassloader.js`, 'utf8', (err, webpackConfTemplateContent) => {
                              if (err) throw err
                                            ...
                                            shell.exec('bit init', { async: true }, (code) => {
                                              ...
                                              fs.readFile(`${currentPath}/bit.json`, 'utf8', (err, content) => {
                                                ...
                                                fs.writeFile(`${currentPath}/bit.json`, newContent, (err) => {
                                                  if (err) throw err
                                                  importBitComponent(`${currentPath}/src`, 'global', 'style', null, (err) => {
                                                    if (err) throw err
