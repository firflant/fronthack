...
const purgeStyleguide = require('../helpers/purgeStyleguide')
              ...
              fs.readdir(`${fronthackPath}/src/sass/components`, (err, files) => {
                ...
                fs.readFile(`${fronthackPath}/src/sass/app.sass`, 'utf8', (err, data) => {
                  ...
                  files.forEach((file) => {
                    ...
                    fs.remove(`${fronthackPath}/src/sass/components/${file}`, (err) => {
                      if (err) throw err
                    })
                  })
                  fs.writeFile(`${fronthackPath}/src/sass/app.sass`, data, (err) => {
                    ...
                    // Purge styleguide
                    purgeStyleguide(fronthackPath, (err) => {
                      if (err) throw err
                      // Remove GitHub REARME file content and replace it with readme for existing projects.
                      fs.remove(`${fronthackPath}/README.md`, (err) => {
                        if (err) throw err
                        fs.move(`${fronthackPath}/readme-installed.md`, `${fronthackPath}/README.md`, (err) => {
                          if (err) throw err
