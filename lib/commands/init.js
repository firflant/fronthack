        ...
        fs.ensureDir(`${fronthackPath}/src/designs`, (err) => {
          ...
          fs.readFile(`${path}/lib/templates/designs-readme.md`, 'utf8', (err, content) => {
            ...
            fs.writeFile(`${fronthackPath}/src/designs/README.md`, content, (err) => {
              ...
              shell.exec('npm install', { async: true, silent: false }, (code) => {
                if (code != 0) throw new Error('Installing dependencies failed')
                // exit(1)
                // Download global components
                getBitComponentData(fronthackPath, 'global', 'styles', null, () => {
                  if (code != 0) throw new Error('Installing global components failed')
                  getBitComponentData(fronthackPath, 'global', 'form', null, () => {
                    if (code != 0) throw new Error('Installing global components failed')
